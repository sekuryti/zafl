...
main()
{
	...
	if [[ $CICD_NIGHTLY == 1 ]] ; then
		...
		configs="zafl_nostars zafl_opt_graph zafl_domgraph zafl_untracer_critical_edges zafl_context_sensitive_laf_domgraph "
	else
		benchmarks="readelf touch"
		configs="zafl_domgraph zafl_context_sensitive_laf_domgraph"
	fi
